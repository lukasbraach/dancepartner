...
def wasm(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[Path]:
    ...
    draft = tmp_path / "mnt" / "team.draft.yaml"
    monkeypatch.setattr(persistence, "IS_WASM", True)
    monkeypatch.setattr(persistence, "MOUNTPOINT", draft.parent)
    monkeypatch.setattr(persistence, "DRAFT_FILE", draft)
    yield draft
...
def test_the_browser_draft_survives_a_reload(wasm: Path, tiny: Team) -> None:
    ...
    # A reload restarts the Pyodide worker, so only the file on the mount can carry anything.
    assert wasm.exists()
...
def test_an_unmounted_browser_filesystem_is_not_an_error(
    ...
    monkeypatch.setattr(persistence, "DRAFT_FILE", Path("/proc/nonexistent/x/team.draft.yaml"))
...
def test_an_unreadable_draft_is_discarded_not_raised(wasm: Path) -> None:
    ...
    wasm.write_text("this: [is, not, a, team", encoding="utf-8")
    assert persistence.load_draft() is None
